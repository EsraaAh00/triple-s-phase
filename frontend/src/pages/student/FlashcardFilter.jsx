...
const FlashcardFilter = () => {
  ...
  const fetchChapters = async () => {
    ...
    try {
      ...
      for (const productId of selectedProducts) {
        const response = await assessmentService.getChapters(productId, 'flashcard');
        if (response.success) {
  ...
  const fetchTopics = async () => {
    ...
    try {
      ...
      for (const chapterId of selectedChapters) {
        const response = await assessmentService.getTopics(chapterId, 'flashcard');
        if (response.success) {
    ...
    try {
      ...
      if (selectedProducts.length > 0) {
        if (selectedTopics.length > 0) {
          // If topics are selected, sum up flashcard counts from selected topics
          for (const topicId of selectedTopics) {
            const topic = topics.find(t => t.id === topicId);
            if (topic && topic.flashcards_count) {
              totalCount += topic.flashcards_count;
            }
          }
        } else if (selectedChapters.length > 0) {
          // If chapters are selected, sum up flashcard counts from selected chapters
          for (const chapterId of selectedChapters) {
            const chapter = chapters.find(c => c.id === chapterId);
            if (chapter && chapter.flashcards_count) {
              totalCount += chapter.flashcards_count;
            }
          }
        } else {
          // If only products are selected, sum up flashcard counts from selected products
          for (const productId of selectedProducts) {
            const product = products.find(p => p.id === productId);
            if (product && product.flashcards_count) {
              totalCount += product.flashcards_count;
            }
          }
        }
      }
  ...
  const handleCreateDeck = () => {
    ...
    // Navigate to flashcard study page with filters
    ...
    navigate(`/student/flashcards/study?${params.toString()}`);
  };
        ...
        <Box sx={{ 
          ...
          <Box sx={{ flex: 1, minWidth: 0 }}>
            ...
            <Typography variant="body2" sx={{ 
              ...
              Create custom flashcard decks for effective learning
            </Typography>
          </Box>
        </Box>
            ...
            <Paper sx={{ 
                    ...
                    <Select
                      ...
                      renderValue={(selected) => {
                        if (selected.length === 0) {
                          return <Typography sx={{ color: '#999', fontSize: { xs: '0.8rem', sm: '0.9rem' } }}>{t('flashcardFilter.subject')}</Typography>;
                        }
                        return (
                ...
                <Box sx={{ flex: 1, minWidth: { xs: '100%', sm: '200px' }, width: { xs: '100%', sm: 'auto' } }}>
                  ...
                  <FormControl fullWidth size="small" disabled={selectedProducts.length === 0}>
                    <Select
                      ...
                      renderValue={(selected) => {
                        if (selected.length === 0) {
                          return <Typography sx={{ color: '#999', fontSize: { xs: '0.8rem', sm: '0.9rem' } }}>{t('flashcardFilter.chapter')}</Typography>;
                        }
                        return (
                          <Typography sx={{ 
                            color: '#333',
                ...
                <Box sx={{ flex: 1, minWidth: { xs: '100%', sm: '200px' }, width: { xs: '100%', sm: 'auto' } }}>
                  ...
                  <FormControl fullWidth size="small" disabled={selectedChapters.length === 0}>
                    <Select
                      ...
                      renderValue={(selected) => {
                        if (selected.length === 0) {
                          return <Typography sx={{ color: '#999', fontSize: { xs: '0.8rem', sm: '0.9rem' } }}>{t('flashcardFilter.topic')}</Typography>;
                        }
                        return (
                          <Typography sx={{ 
                            color: '#333',
              ...
              <Box sx={{ mb: { xs: 3, sm: 4 } }}>
                <Typography variant="body2" sx={{ 
                  ...
                  {t('flashcardFilter.questionCount', 'Number of Questions')}
                </Typography>
                <Box sx={{ px: { xs: 1, sm: 2 } }}>
                  <Slider
                    value={questionCount}
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 1 }}>
                  <Typography variant="body2" sx={{ 
                    ...
                    {t('flashcardFilter.selected', 'Selected')}: {questionCount} {t('commonQuestions', 'Questions')}
                  </Typography>
                </Box>
              </Box>
              ...
              <Box sx={{ 
                ...
                <Box sx={{ flex: 1, mb: { xs: 2, sm: 0 } }}>
                  <Typography variant="h6" sx={{ 
                    ...
                    {t('flashcardFilter.availableCards')}: {loading.count ? (
